(function (root, factory) {
  ...
  function countChars(text) {
    // Spikeでは「改行を除いたUnicodeコードポイント数」。
    // NFKCは互換文字を展開して文字数を変えるため、カウントには使わない。
  ...
  function commonEdges(a, b) {
    // Find common prefix/suffix in code units for speed, then move boundaries away
    // from the middle of a surrogate pair. This avoids Array.from() on an entire
    // multi-hundred-thousand-character article when only a small region changed.
    var start = 0;
    while (start < a.length && start < b.length && a.charCodeAt(start) === b.charCodeAt(start)) start++;
    if (start > 0 && start < a.length && start < b.length &&
        isHighSurrogate(a.charCodeAt(start - 1)) && isLowSurrogate(a.charCodeAt(start))) {
      start--;
    }
    ...
    if (endA < a.length && endB < b.length && endA > start && endB > start &&
        isLowSurrogate(a.charCodeAt(endA)) && isHighSurrogate(a.charCodeAt(endA - 1))) {
      endA--;
      endB--;
    }
  ...
  function computeDeltaStats(a, b) {
    ...
    return {
      removed: countCodePoints(a.slice(edges.start, edges.endA)),
      added: countCodePoints(b.slice(edges.start, edges.endB))
    };
  }
  ...
  function lcsOps(aTokens, bTokens) {
    ...
    var j2 = 0;
    while (i2 < n && j2 < m) {
      if (aTokens[i2] === bTokens[j2]) {
        ops.push({ type: 'same', ch: aTokens[i2] });
        i2++;
        j2++;
      } else if (dp[(i2 + 1) * width + j2] >= dp[i2 * width + j2 + 1]) {
        ops.push({ type: 'del', ch: aTokens[i2++] });
      } else {
        ops.push({ type: 'add', ch: bTokens[j2++] });
      }
    }
  ...
  function parseNoteRoute(input) {
    ...
    if (host !== 'note.com' && host !== 'editor.note.com') return null;

    var path = url.pathname.replace(/\/+$/, '') || '/';
    var edit = path.match(/^\/notes\/([^/]+)\/edit$/i);
    if (edit && edit[1] !== 'new') {
      return { kind: 'article', articleId: edit[1], routeKey: 'article:' + edit[1] };
    }

    if (path === '/notes/new' || path === '/new') {
      return { kind: 'new', articleId: null, routeKey: 'new' };
    }
    return null;
  }
